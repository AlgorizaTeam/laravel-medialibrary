...
The event has a property `media` that holds the `\AlgorizaTeam\MediaLibrary\Models\Media`-object of which the file has been stored.
...
- `media`: the `\AlgorizaTeam\MediaLibrary\Models\Media`-object of which a conversion will be started
- `conversion`: the conversion (an instance of `\AlgorizaTeam\MediaLibrary\Conversion\Conversion`) that will start
...
- `media`: the `\AlgorizaTeam\MediaLibrary\Models\Media`-object of which a conversion has been completed
- `conversion`: the conversion (an instance of `\AlgorizaTeam\MediaLibrary\Conversion\Conversion`) that has just been completed
...
- `model`:  the object that conforms to `\AlgorizaTeam\MediaLibrary\HasMedia\Interfaces\HasMedia` of which a collection has just been cleared.
...
```php
protected $listen = [
    'AlgorizaTeam\MediaLibrary\MediaCollections\Events\MediaHasBeenAdded' => [
